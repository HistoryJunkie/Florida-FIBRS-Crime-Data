...
BODY_WITH_DATA = """  <div class="stat-row">
  ...
  <div class="section">
    ...
    <p class="trend-caption" id="trendCaption">Total offenses reported per year. Striped = partial or in-progress year. Gray = no submitted data that year.</p>
  </div>
...
const chartW = 800, chartH = 260;
const padL = 36, padR = 12, padT = 12, padB = 24;
const plotW = chartW - padL - padR;
const plotH = chartH - padT - padB;

function renderTrendAllTime(){
  const barPadT = 28, barPadB = 44;
  const barPlotH = chartH - barPadT - barPadB;
  const barGap = 20;
  const barW = (plotW - barGap * (years.length - 1)) / years.length;
  const MIN_BAR_PX = 5;

  const yearInfo = years.map(year => {
    const rowValues = grid[year];
    const known = rowValues.filter(v => v !== null).length;
    const submitted = rowValues.filter(v => v !== null && v > 0).length;
    const total = rowValues.reduce((s, v) => s + (v || 0), 0);
    return {year, known, submitted, total};
  });

  const maxTotal = Math.max(...yearInfo.map(y => y.total), 1);

  function heightFor(total){
    if(total <= 0) return 0;
    return Math.max(MIN_BAR_PX, (total / maxTotal) * barPlotH);
  }

  const yTicks = [0, Math.round(maxTotal/2), maxTotal];
  const yTickEls = yTicks.map(t => {
    const y = barPadT + barPlotH - (t / maxTotal) * barPlotH;
    return `<line x1="${padL}" y1="${y}" x2="${chartW-padR}" y2="${y}" stroke="#30363d" stroke-width="1" stroke-dasharray="2,3"/>
            <text x="${padL-8}" y="${y+4}" fill="#8b949e" font-size="10" text-anchor="end">${t.toLocaleString()}</text>`;
  }).join('');

  const barEls = yearInfo.map((info, i) => {
    const x = padL + i * (barW + barGap);
    const centerX = x + barW / 2;

    let h, fillAttr, labelTop, labelSub, subColor;
    if(info.submitted === 0){
      h = MIN_BAR_PX;
      fillAttr = 'fill="#21262d"';
      labelTop = null;
      labelSub = 'no data';
      subColor = '#8b949e';
    } else {
      h = heightFor(info.total);
      const isFullYear = info.known === 12 && info.submitted === 12;
      fillAttr = isFullYear ? 'fill="#39d353"' : 'fill="url(#allTimeStripe)"';
      labelTop = info.total.toLocaleString();
      if(isFullYear){
        labelSub = null;
      } else if(info.known < 12){
        labelSub = `${info.submitted} of ${info.known} mo. so far`;
        subColor = '#f85149';
      } else {
        labelSub = `${info.submitted} of 12 mo.`;
        subColor = '#f85149';
      }
    }

    const y = barPadT + barPlotH - h;
    const tooltip = `${info.year}: ${info.total.toLocaleString()} offenses (${info.submitted} of ${info.known} months submitted)`;
    let svg = `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${barW.toFixed(1)}" height="${h.toFixed(1)}" rx="3" ${fillAttr}><title>${tooltip}</title></rect>`;
    if(labelTop){
      svg += `<text x="${centerX.toFixed(1)}" y="${(y-8).toFixed(1)}" fill="#c9d1d9" font-size="12" text-anchor="middle">${labelTop}</text>`;
    }
    svg += `<text x="${centerX.toFixed(1)}" y="${(barPadT+barPlotH+20).toFixed(1)}" fill="#c9d1d9" font-size="12" text-anchor="middle">${info.year}</text>`;
    if(labelSub){
      svg += `<text x="${centerX.toFixed(1)}" y="${(barPadT+barPlotH+34).toFixed(1)}" fill="${subColor}" font-size="10" text-anchor="middle">${labelSub}</text>`;
    }
    return svg;
  }).join('');
  ...
  document.getElementById('trendChart').innerHTML = `
    <svg class="trend-svg" viewBox="0 0 ${chartW} ${chartH}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <pattern id="allTimeStripe" width="8" height="8" patternTransform="rotate(45)" patternUnits="userSpaceOnUse">
          <rect width="8" height="8" fill="#0e4429"/>
          <rect width="4" height="8" fill="#39d353"/>
        </pattern>
      </defs>
      ${yTickEls}
      ${barEls}
...
const offenseTotalsAllTime = __OFFENSE_JS__;
const offenseTotalsByYear = __OFFENSE_BY_YEAR_JS__;
...
function applyYearFilter(year){
  ...
  if(year === 'all'){
    ...
    trendCaption.textContent = 'Total offenses reported per year. Striped = partial or in-progress year. Gray = no submitted data that year.';
    renderTrendAllTime();
    if(offenseHeading){
...
def build_page(agency_name, county, agency_data, dataset_start, dataset_end, month_labels):
    ...
    if stats is None:
        ...
        offense_types_by_year = agency_data.get("offense_types_by_year", {})
        ...
        body = body.replace("__OFFENSE_BY_YEAR_JS__", json.dumps(offense_types_by_year))
